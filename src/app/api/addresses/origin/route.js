...
export async function POST(request) {
  ...
  let createdAddress;
  try {
    const existingAddresses = await prisma.originAddress.findMany();

    createdAddress = await prisma.originAddress.create({
      data: {
        ...
        is_active: existingAddresses.length === 0 ? true : false,
      },
    });
  } catch (e) {
